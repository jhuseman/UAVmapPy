...
class ObjectDetector(object):
	...
	def object_detect(self, image, alg="closing_color"):
		...
		def color_detect(image):
			...
			boundaries = [
				...
				([125, 75, 50], [255, 250, 150]),
				([120, 25, 60], [255, 250, 100]),
			]
		...
		def closing_color(image):
			...
			# return color_detect(cv2.morphologyEx(image, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_RECT,(100,100))))
		def closing_color2(image):
			# define the list of boundaries
			boundaries = [
				([17, 15, 100], [50, 56, 200]),
				([86, 31, 4], [220, 88, 50]),
			...
			out = color_detect(cv2.morphologyEx(output, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_RECT,(200,200))))

			plt.imshow(out)
			plt.show()
			return big_contour(out)
		return {
			"big_contour":big_contour,
			"color_detect":color_detect,
			"closing_color":closing_color,
			"closing_color2":closing_color2,
			}[alg](image)
	...
	def crop(self, image, rect):
		...
		box_overlay = cv2.rectangle(image,(rect[0],rect[1]),(rect[0]+rect[2],rect[1]+rect[3]),(0,255,0),8)
		plt.imshow(box_overlay)
		plt.show()
		# print(rect)
		...
		ret = image[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]]
		plt.imshow(ret)
		plt.show()
		return ret
